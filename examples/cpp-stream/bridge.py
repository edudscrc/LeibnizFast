...
Receives binary frames from the C++ generator over ZMQ PUSH/PULL and
rebroadcasts each message verbatim (binary) to all connected WebSocket
clients. Also forwards resize commands from browser clients back to the
generator via a second ZMQ PUSH socket.

ZMQ sockets:
  tcp://127.0.0.1:5555  PULL — receives simulation frames from generator
  tcp://127.0.0.1:5556  PUSH — sends resize control commands to generator
...
Usage:
    ...
    python bridge.py
    python bridge.py --debug    # enable per-frame performance logging
"""
                    ...
                    try:
                        ...
                        if data.get("type") == "resize":
                            ...
                            if 4 <= new_size <= 8192:
                                ...
                                await ctrl_sock.send(struct.pack("<I", new_size))
                                log.info("Forwarded resize → %d", new_size)
...
async def zmq_loop(data_sock: zmq.asyncio.Socket) -> None:
    """Pull frames from ZMQ and broadcast binary payload to all clients."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ZMQ → WebSocket bridge for cpp-stream example")
    parser.add_argument("--debug", action="store_true", help="Enable per-frame performance logging")
    args = parser.parse_args()

    DEBUG = args.debug
    if DEBUG:
        log.info("Debug mode enabled — per-frame performance logging active")
